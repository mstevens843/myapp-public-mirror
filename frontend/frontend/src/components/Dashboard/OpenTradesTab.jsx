...
// July 2025 — glow pass (neutral pills removed)
...
const loadTpSl = async () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      // console.log("📦 /positions response", posSnap.positions.find(p => p.mint.includes("Df6yfrK")));
...
// const posMatch =
//   posArr.find((p) => p.mint === b.mint && p.strategy === b.strategy) || {};
const posMatch =
...
return {
 ...
 /* fetch on mount */
  ...
  /* sell */
  // const sell = async (mint, strategy, frac) => {
...
const hasDust = merged.some(r => (r.tokens <= 0 || r.tokens < 0.000001));
  // const showTpSl = visible.some(r => tpSl[`${r.mint}_${r.strategy}`]);
  const showTpSl = true;
const totalUsd = merged.filter(r => r.strategy !== "paperTrader")   // ← exclude fake capital
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm text-center">
            ...
            <tbody className="divide-y divide-zinc-800">
              {loading && (
                <tr>
                  <td colSpan={showTpSl?7:6} className="p-6">
              ...
              {!loading && !visible.length && (
                <tr>
                  <td colSpan={showTpSl?7:6} className="p-6 text-center italic text-zinc-500">
                    No open positions.
                  </td>
                </tr>
              )}
...
{/* {showTpSl && (
  <td className="py-2 px-3">
    {tpSl[`${r.mint}_${r.strategy}`]
      ? <TpSlCell
          mint={r.mint}
          strategy={r.strategy}
          settings={tpSl[`${r.mint}_${r.strategy}`]}
          onSaved={refresh}/>
