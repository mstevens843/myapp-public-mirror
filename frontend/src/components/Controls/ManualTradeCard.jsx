...
export default function ManualTradeCard({
  ...
  balanceGlow,
  lastBalanceUpdate,
  lastBlockedMint,     // kept in props for compatibility; not used
  setLastBlockedMint,  // kept in props for compatibility; not used
  walletId,
}) {
  /* ---- target token (no external fetch needed) ---- */
  // Prefer an explicit targetToken if present (string or {mint}),
  // then fall back to tokenMint/outputMint on the config.
  const targetMint =
    (typeof swapOpts?.targetToken === "string"
      ? swapOpts?.targetToken
      : swapOpts?.targetToken?.mint) ||
    config?.tokenMint ||
    config?.outputMint ||
    null;
    ...
    >
      ...
      <div className="relative mb-1">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span
              className={
                selectedWalletBalance > 0.1
                  ? "text-emerald-400"
                  : selectedWalletBalance > 0.09
                  ? "text-yellow-400"
                  : "text-red-400"
              }
            >
          ...
          {targetMint && (
            ...
            >
              <img
                src={`https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/solana/assets/${targetMint}/logo.png`}
                onError={(e) => (e.currentTarget.style.display = "none")}
                alt="token"
                className="w-4 h-4 rounded-full"
              />
              🎯 {targetMint.slice(0, 6)}…{targetMint.slice(-4)}
            </span>
          )}
      ...
      {quotePreview && (
        <div className="text-xs text-zinc-400 italic mb-4">
          ...
          <span className={`font-semibold ${quotePreview.priceImpact > 2 ? "text-red-400" : "text-emerald-400"}`}>
      ...
      <LimitModal open={limitModalOpen} onClose={() => setLimitModalOpen(false)} walletId={walletId} tokenMint={config.tokenMint} />
      <DcaModal open={dcaModalOpen} onClose={() => setDcaModalOpen(false)} walletId={walletId} tokenMint={config.tokenMint} />
    </motion.div>
  );
}
