...
/** Rotation Bot Strategy Module
 * - Rotates capital into the best performing token in a monitored list. 
 * - Now supports optional *sector rotation* with momentum decay & per‑token cooldown.
 * 
 */

/* rotationBot.js  🚦 selects strongest-performer every rotation (with optional sector mode) */
...
// /* ───────────────── helper: sector scoring (optional) ─────────────── */
// /**
//  * Compute sector → avg momentum and return { sector, score, topToken, tokenScores }
...
//       // ── Sector mode pre-compute (optional, global once per tick) ──
//       let sectorPick = null;
//       if (ENABLE_SECTOR) {
...
//           log("info", `🏷️ Sector mode: best sector = ${sectorPick.sector}, avg ${(sectorPick.score*100).toFixed(2)}%`);
//           log("info", `🏆 Sector mode: top token = ${sectorPick.topToken?.slice(0,4)}…`);
//         }
//       }
...
//         if (!tokenUniverse.length) { log("warn", `[${label}] no tokens configured — skip`); continue; }
